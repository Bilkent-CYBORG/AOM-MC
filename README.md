...
Official implementation of Online Context-Aware Task Assignment in Mobile Crowdsourcing via Adaptive Discretization

This repository is the official implementation of Contextual Combinatorial Volatile Bandits via Gaussian Processes, submitted to NeurIPS21.
...
We ran a total of three simulations, one presented in the main paper and two presented in the supplemental. Moreover, none of the algorithms that we implement and test do offline-learning, thus there is no 'training' to be done. However, to be able to repeat the simulations and also improve speed, we first generate the arm contexts, rewards, and other setup-related information and save them as HDF5, in the case of the main simulation, and pickled DataFrames, in the case of the supplemental simulations. We provide the links to the generated datasets that we used at the bottom of this README file. By default, when you run the script (main.py), it re-generates new datasets and runs the simulations on them.

### Main paper simulations (DPMC)
To run the main paper simulations, provide the argument `main` to the main.py script. For example, to re-generate the main paper simulation datasets and run the simulations on the newly generated datasets use
...
```
### Supplemental simulations I (Foursquare)
To run the first supplemental simulation, use the argument `supp_1`. Similar to the main paper simulation code, you can provide the `--use_saved_dataset` argument to use pre-generated and saved datasets.
### Supplemental simulations II (Varying kernel parameters)
To run the second supplemental simulation, use the argument `supp_2`. Similar to the main paper simulation code, you can provide the `--use_saved_dataset` argument to use pre-generated and saved datasets.
...
### Supplemental simulation I datasets
A zip file of the pickled DataFrames used for the Foursquare supplemental simulation can be downloaded [here](https://api.onedrive.com/v1.0/shares/u!aHR0cHM6Ly8xZHJ2Lm1zL3UvcyFBdjctellURkxybzBycnhjUmJBamFwLV8wQ010aVE_ZT1sWmJWODA/root/content). Make sure to extract both 'fs_tky_simulation_df_uni' and 'fs_tky_simulation_df_nuni' and place them in the root directory where main.py is.
